...
impl FBDisplay {
    pub fn new(width: usize, height: usize, scale: f32, buffer: Arc<Mutex<Vec<u8>>>, event_receiver: mpsc::Receiver<Event>) -> FBDisplay {
    ...
    pub fn run(&mut self) {
        ...
        loop {
            ...
            self.window.update_with_buffer(&dsp_buffer, self.width, self.height).expect("Could not update display");
        }
    }
}