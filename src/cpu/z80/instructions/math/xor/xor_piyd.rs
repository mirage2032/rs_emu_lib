...
impl XOR_PIYD {
    pub fn new(memory: &dyn MemoryDevice, pos: u16) -> Result<XOR_PIYD, MemoryReadError> {
        Ok(XOR_PIYD {
    ...
    pub fn new_with_value(d: u8) -> XOR_PIYD {
        XOR_PIYD {
            common: InstructionCommon::new(3, 19, true),
            d: d as i8,
        }
...
impl Display for XOR_PIYD {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "XOR (IY+0x{:02X})", self.d as u8)
    }
}

impl BaseInstruction for XOR_PIYD {
    fn common(&self) -> &InstructionCommon {
        &self.common
    }
    fn to_bytes(&self) -> Vec<u8> {
        vec![0xfd, 0xae, self.d as u8]
    }
}

impl ExecutableInstruction<Z80> for XOR_PIYD {
