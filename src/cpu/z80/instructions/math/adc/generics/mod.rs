...
macro_rules! adc_rr_rr_setf {
    ($reg1:expr, $reg2:expr, $flags:expr) => {
        ...
        // calculate flags
        let carry = (value_before as u32 + reg2 as u32 + $flags.carry() as u32) > 0xFFFF;
        let half_carry = (value_before & 0x0fff) + (reg2 & 0x0fff) + $flags.carry() as u16 > 0x0fff;
        let zero = result == 0;
        let sign = (result & 0x8000) != 0;
        let overflow =
            (value_before & 0x8000) == (reg2 & 0x8000) && (value_before & 0x8000) != (result & 0x8000);
        *$reg1 = result;
