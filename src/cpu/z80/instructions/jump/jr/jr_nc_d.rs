...
impl JR_NC_D {
    pub fn new(memory: &dyn MemoryDevice, pos: u16) -> Result<JR_NC_D, MemoryReadError> {
        Ok(JR_NC_D {
    ...
    pub fn new_with_value(d: u8) -> JR_NC_D {
        JR_NC_D {
            common: InstructionCommon::new(2, 7, true),
            d: d as i8,
        }
...
impl Display for JR_NC_D {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "JR NC, 0x{:02x}", self.d)
    }
}

impl BaseInstruction for JR_NC_D {
    fn common(&self) -> &InstructionCommon {
        &self.common
    }
    fn to_bytes(&self) -> Vec<u8> {
        vec![0x30, self.d as u8]
    }
}
...
mod tests {
    ...
    test_z80!("30");
    test_instruction_parse!(JR_NC_D, [0xbf]);
}
