...
use crate::emu_lib::cpu::{BaseInstruction, ExecutableInstruction};
...
pub fn encode(instruction: String) -> Result<Box<dyn BaseInstruction>, String> {
    ...
    let instruction: Box<dyn BaseInstruction> = match op.get(1).unwrap().as_str() {
