...
fn assert_z80(emulator: &mut Emulator,test_state: &TestState){
    ...
    assert_eq!(emulator.io.iff1,if test_state.iff1==1{true} else {false});
    assert_eq!(emulator.io.iff2,if test_state.iff2==1{true} else {false});
...
macro_rules! include_test_data {
    ($test_data_path:literal ) => {
        {
            let test_data = include_str!(concat!(env!("CARGO_MANIFEST_DIR"),"/tests/z80/v1/",$test_data_path));
            let test_data: Vec<TestData> = serde_json::from_str(test_data).expect("Failed to parse test data");
            test_data
        }
    };
}
